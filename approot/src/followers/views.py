...
class FollowerAPIView(APIView):
    ...
    def post(self, request, pk):
        ...
        return Response(data={"details": f"{request.user} started follow {user}"}, status=status.HTTP_201_CREATED)

    def delete(self, request, pk):
        user = User.objects.get(id=self.kwargs['pk'])
        Follower.objects.get(user=user, follower=request.user).delete()
        return Response(data={"details": f"{request.user} no more follows {user}"}, status=status.HTTP_204_NO_CONTENT)
